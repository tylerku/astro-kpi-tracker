...
const BarGraph: React.FC<BarGraphProps> = (props) => {
  ...
  const getBars = () => {
    const selectedOption = getSelectedOption()
    const heightClass = percentageHeightClasses[getGraphDataPointHeightPercentage({title: '', value: 8}, 10)]
    console.log('getBars heightClass: ', heightClass)
    ...
    const getBarBackgroundColor = (barValue: number) => {
      if (barValue === completionPercentage) return backgroundColors.green
      if (barValue >= completionPercentage * .50) return backgroundColors.yellow
      if (barValue >= completionPercentage * 0) return backgroundColors.orange
      return backgroundColors.red
    }

    if (!selectedOption) return []
    const bars = selectedOption.data?.map((dataPoint: GraphDataPoint) => {
      return (
        <div className='flex-grow h-full flex flex-col justify-end items-center text-sm pb-4 relative'>
          <div className={`transition-all duration-500 w-full max-w-[30px] ${percentageHeightClasses[getGraphDataPointHeightPercentage(dataPoint, selectedOption.maxY)]} ${getBarBackgroundColor(dataPoint.value)} rounded-3xl`} />
